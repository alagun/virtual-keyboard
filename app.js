...
<div class="keys">
  ...
  // определение языка
  const currentLangObj = () => { if (localStorage.getItem('lang') === 'en') { return en; } return ru; };
  let lang = currentLangObj();
  ...
  const createKeyboard = (language, modification) => {
    const rowArr = document.querySelectorAll('.row');
    const currObj = currentLangObj();
    currObj[modification].forEach((el, ind) => {
      const symbolArr = el.split(' ');
  ...
  const mouseClickKey = (event) => {
    ...
    if (/ArrowUp/.test(key.dataset.code)) {
      const arrStr = textarea.value.split('\n').map((e) => e.length);
      let sumLenStr = 0;
      for (let strNumber = 0; strNumber < arrStr.length; strNumber += 1) {
        sumLenStr
